    ...
    for num in range(pl_num):
        while True:
            ...
            if np.linalg.norm(np.array([Nx, Ny]) - np.array([32.5, 32.5])) < 4:
                ...
                planet_pos.append((Nx,Ny))
                break
    return image, pos_label, planet_pos

def data_preprocess(data,psf_pl,c_ratio,no_blend):

    ## inject planet for train_data
    injected_samples = np.zeros([len(data), 64, 64])
    planet_masks = np.zeros([len(data)*2, 64, 64])
    planet_positions = []
    for i in range(len(data)):
        new_img, mask_planet, planet_pos = inject_planet(data[i].reshape(64, 64), psf_pl, c_ratio=c_ratio,no_blend=True)
        injected_samples[i] += new_img
        planet_masks[i] += mask_planet
        planet_positions.append(planet_pos)

    normalised_injected = local_normal(injected_samples)
    nor_data = local_normal(data)
    dataset = np.zeros([int(len(data)), 64, 64])
    ...
    return dataset.reshape(-1,64,64,1),label,planet_masks,planet_positions
